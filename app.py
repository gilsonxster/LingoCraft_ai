...
<style>
    ...
    /* Disable tooltip popups in sidebar to prevent blocking button interactions */
...
def select_tense_and_coach(idx: int):
    st.session_state.active_tense_index = idx
    safe_set_main_tab(MAIN_TAB_COACH)
    auto_save_current_session()
...
@st.fragment
def _incontext_coach_fragment(current_tense_name: str, step_num: int, prompts: list, target_l: str, native_l: str):
    """
    Isolated Streamlit fragment for in-context coach queries.
...
@st.fragment
def _speech_practice_fragment(current_tense_name: str, pack, target_l: str, native_l: str):
    """
    Isolated Streamlit fragment for microphone recording, audio playback,
    transcription, and speech evaluation.
    Interactions (stopping recording, evaluating) run in-place without refreshing the full page.
    """
    ...
    with voice_tab1:
        try:
            ...
            if audio_record:
                ...
                target_code = st.session_state.current_curriculum.get("target_language_code", "es")
                transcribed = transcribe_audio_bytes(audio_record['bytes'], lang=target_code)
                if transcribed:
                    ...
                    if st.button("Evaluate recorded speech", key=f"eval_rec_{current_tense_name}"):
                        ...
                        with st.spinner("LingoCraft AI Speech Validator analyzing phonetics..."):
                            speech_orch = LingoCraftOrchestrator(api_key=st.session_state.get("api_key", os.getenv("GEMINI_API_KEY", "")))
                            eval_res = speech_orch.validate_speech(
    ...
    with voice_tab2:
        clean_speech_val = re.sub(r'[*_~`]', '', pack.card4_speech.target_phrase).strip()
        spoken_input = st.text_input("Type or confirm what you said:", value=clean_speech_val, key=f"text_speech_{current_tense_name}")
    ...
    if st.session_state.speech_eval_result:
        ...
        fb_title = html.escape(str(res.get('feedback_title', '¡Excelente!')))
        raw_msg = str(res.get('evaluation_message', ''))
        fb_msg = re.sub(r'\*\*(.*?)\*\*', r'<strong>\1</strong>', html.escape(raw_msg))
        raw_tip = str(res.get('actionable_tip', ''))
        fb_tip = re.sub(r'\*\*(.*?)\*\*', r'<strong>\1</strong>', html.escape(raw_tip))

        if score >= 75:
            st.markdown(f"<div class='feedback-box-success' role='status' aria-live='polite'><h4>{fb_title}</h4><p>{fb_msg}</p><p><strong>Tip:</strong> {fb_tip}</p></div>", unsafe_allow_html=True)
        else:
            st.session_state.needs_review_tenses.add(st.session_state.active_tense_index)
            st.markdown(f"<div class='feedback-box-error' role='alert' aria-live='assertive'><h4>{fb_title}</h4><p>{fb_msg}</p><p><strong>Adjustment:</strong> {fb_tip}</p></div>", unsafe_allow_html=True)
...
if "pack_cache" not in st.session_state:
    st.session_state.pack_cache = {}
    ...
    with st.popover("📚 Switch Topic / Language", use_container_width=True):
        ...
        if study_mode == "Curated Core Topics":
            ...
            if not st.session_state.api_key:
                st.caption("ℹ️ *Enter a Gemini API Key on the sidebar below to generate custom AI topics.*")
...
if selected_tab == MAIN_TAB_CURRICULUM:
    ...
    for i, t in enumerate(roadmap):
        ...
        col_study, col_opts = st.columns([3.6, 1.2])
        with col_study:
    ...
    if is_curriculum_completed:
        ...
        <div style="background: linear-gradient(135deg, #1A73E8 0%, #137333 100%); border-radius: 14px; padding: 24px 28px; color: white; margin-bottom: 20px; box-shadow: 0 4px 12px rgba(26, 115, 232, 0.15);">
            <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 16px;">
                ...
                <div style="text-align: right; min-width: 140px; background: rgba(255,255,255,0.18); padding: 12px 18px; border-radius: 12px; backdrop-filter: blur(4px);">
                    ...
                    <div style="font-size: 0.8rem; font-weight: 600;">{curr_rank.get('label', '')}</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        pack_topic = st.session_state.current_curriculum.get("topic", "")
        target_lang = st.session_state.current_curriculum.get("target_language", "Spanish")
        native_lang = st.session_state.current_curriculum.get("native_language", "English")
        pack_key = f"{pack_topic}::{current_tense_name}::{native_lang}"

        def _is_invalid_pack(p, expected_tense):
            if not p or not getattr(p, "card1_concept", None):
                return True
            if not getattr(p.card1_concept, "conjugations", None):
                return True
            c1_title = (getattr(p.card1_concept, "title", "") or "").lower()
            exp_tense = (expected_tense or "").lower()
            if "infinitivo" not in exp_tense and "infinitivo" in c1_title:
                return True
            return False

        cached_pack = st.session_state.pack_cache.get(pack_key)
        if cached_pack and _is_invalid_pack(cached_pack, current_tense_name):
            cached_pack = None
            if pack_key in st.session_state.pack_cache:
                del st.session_state.pack_cache[pack_key]

        if cached_pack:
            st.session_state.current_pack = cached_pack
        elif (
            st.session_state.current_pack is None
            or st.session_state.current_pack.tense_name != current_tense_name
            or _is_invalid_pack(st.session_state.current_pack, current_tense_name)
        ):
            with st.spinner(f"Generating 5-Stage Flashcard Pack for **{current_tense_name}**..."):
                ...
                st.session_state.pack_cache[pack_key] = pack
